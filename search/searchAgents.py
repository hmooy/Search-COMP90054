...
class DeceptiveSearchAgentpi2(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        start = currentState.getPacmanPosition()
        foods = currentState.getFood().asList()
        optcGr_Gi = [len(mazeDistanceC(foods[0],cap,state)) for cap in capsules]
        optcS_Gr = [len(mazeDistanceC(start,food,state)) for food in foods]
        optcS_Gi = [len(mazeDistanceC(start,cap,state)) for cap in capsules]

        beta_min = min([(a+b-c)/2 for a,b,c in zip(optcGr_Gi, optcS_Gr,optcS_Gi)])
        beta_min = int(beta_min)

        pathGr_Gi = mazeDistanceD(foods[0],capsules[0],state)
        t = (pathGr_Gi[beta_min-1])

        s_t = mazeDistanceC(start, t, state)
        t_gr = mazeDistanceC(t, foods[0], state)

        self.actions = self.actions + s_t + t_gr
        return self.actions
...
class DeceptiveSearchAgentpi3(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        s_t = mazeDistanceE(start, t, state, foods[0], capsules[0], beta_min)
        t_gr = mazeDistanceC(t, foods[0], state)
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    """
    Find the closest food distance by using A*
    """

    foods = foodGrid.asList()
    trackCap = list(capsules)
    if len(foods) == 0:
        return 0
    if len(foods) == 1:
        return mazeDistanceP(position, foods[0], problem.startingGameState, trackCap)

    closest = float('inf')
    closest_f = foods[0]
    maximum = 0
    for food in foods:
        dist = mazeDistanceP(position, food, problem.startingGameState, trackCap)
        if dist < closest:
            closest = dist
            closest_f = food
        if dist > maximum:
    ...
    closest_max = mazeDistanceP(closest_f, max_f, problem.startingGameState, trackCap)
    return closest+closest_max

    # j = 1
    # max = 0
    # for i in foods:
    #     for j in range(len(foods)):
    #         c = len(mazeDistanceP(i, foods[j], problem.startingGameState, trackCap))
    #         if c > max:
    #             max = c
    #             food1 = i
    #             food2 = foods[j]
    #
    # x = min(len(mazeDistanceP(position, food1, problem.startingGameState, trackCap)),len(mazeDistanceP(position, food2, problem.startingGameState, trackCap)))
    # h = max + x
    #
    # h_2 = 0
    # while foods:
    ...
    #     closest_food = None
    #     for food in foods:
    #         path = mazeDistanceP(position, food, problem.startingGameState, trackCap) #find distance between position and food
    #         path_cost = len(path)
    #         if path_cost < cost:
    #             cost = path_cost #set lowest cost
    #             closest_food = food #food position
    #             closest_path = path #the path to food
    #
    #     position = closest_food #new pacman position
    #     trackCap = list(set(trackCap)-set(closest_path)) #remove used capsule
    #     h_2 = h_2 + cost #adding all cost to complete goal
    #     foods.remove(closest_food) #remove food you ate
    # print(min(h,h_2))
    # return min(h, h_2)
...
def mazeDistanceP(point1, point2, gameState, trackCap):
    """
    Returns the maze distance between any two points, using the search functions
    you have already built. The gameState can be any game state -- Pacman's
    position in that state is ignored.
    ...
    assert not walls[x2][y2], 'point2 is a wall: ' + str(point2)
    prob = PositionSearchProblem(gameState, start=point1, goal=point2, warn=False, visualize=False)
    return search.astarp(prob, trackCap)

def mazeDistanceC(point1, point2, gameState):
    """
    Returns the maze distance between any two points, using the search functions
    you have already built. The gameState can be any game state -- Pacman's
    position in that state is ignored.
...
def mazeDistanceD(point1, point2, gameState):
    """
    Returns the maze distance between any two points, using the search functions
    you have already built. The gameState can be any game state -- Pacman's
    position in that state is ignored.
    ...
    assert not walls[x2][y2], 'point2 is a wall: ' + str(point2)
    prob = PositionSearchProblem(gameState, start=point1, goal=point2, warn=False, visualize=False)
    return search.aStarSearchP2(prob)


def mazeDistanceE(point1, point2, gameState, gr, gmin, betamin):
    """
    Returns the maze distance between any two points, using the search functions
    you have already built. The gameState can be any game state -- Pacman's
    position in that state is ignored.
    ...
    assert not walls[x2][y2], 'point2 is a wall: ' + str(point2)
    prob = PositionSearchProblem(gameState, start=point1, goal=point2, warn=False, visualize=False)
    return search.aStarSearchP42(prob, gr, gmin, betamin)